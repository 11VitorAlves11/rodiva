    ...
    <div className="min-w-0 space-y-5">
      ...
      {items.length === 0 ? <p className="rounded-xl border border-dashed border-line-strong bg-raised p-8 text-center text-sm text-ink-subtle">{t("inventory.empty")}</p> : (
        <div className="grid min-w-0 gap-3 sm:grid-cols-2 xl:grid-cols-3">{items.map((item) => (
          <article key={item.id} className={`min-w-0 rounded-xl border bg-raised p-4 shadow-sm ${item.low_stock ? "border-amber-400 dark:border-amber-700" : "border-line"}`}>
            <div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><h2 className="break-words font-semibold text-ink">{item.name}</h2><p className="truncate text-xs text-ink-subtle">{[item.manufacturer, item.reference].filter(Boolean).join(" · ") || vehicleName(item.vehicle_id)}</p></div>{item.low_stock && <span className="shrink-0 rounded-full bg-amber-100 px-2 py-1 text-[10px] font-bold uppercase text-amber-900 dark:bg-amber-950 dark:text-amber-200">{t("inventory.lowStock")}</span>}</div>
